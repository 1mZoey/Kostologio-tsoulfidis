  ...
  const toggleAll = () => {
    setSelected(
      selected.size === entries.length
        ? new Set()
        : new Set(entries.map((e) => e._id?.toString())),
    );
  };
        ...
        {loading ? (
          ...
          <table className='w-full'>
            <thead className='bg-gray-50 dark:bg-[#21252b] border-b border-gray-200 dark:border-[#181a1f]'>
              <tr className={`transition-colors hover:... align-middle`}>
        ...
        selected.size === entries.length && entries.length > 0
          ...
          : "border-gray-300 dark:border-gray-600 bg-transparent"
                ...
                {[
                  ...
                  <th
                    key={h}
                    className='text-left px-4 py-5 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 whitespace-nowrap'
                  >
                    {h}
                  </th>
                ))}
        ...
        isSelected
          ...
          : "border-gray-300 dark:border-gray-600 bg-transparent"
                    ...
                    <td className='px-4 py-5 font-medium text-gray-900 dark:text-gray-100 min-w-[150px]'>
                      {e.name || (
                        <span className='text-gray-400 dark:text-gray-500 italic font-normal text-sm'>
                          Χωρίς όνομα
                        </span>
                      )}
                    </td>
                    <td className='px-4 py-5 text-gray-600 dark:text-gray-300 text-sm whitespace-nowrap'>
                      {e.inputs?.productName || "—"}
                    </td>
                    <td className='px-4 py-5 text-gray-500 dark:text-gray-400 text-sm whitespace-nowrap'>
                      {e.inputs?.source || "—"}
                    </td>
                    <td className='px-4 py-5 text-gray-500 dark:text-gray-400 text-sm whitespace-nowrap'>
                      {e.inputs?.quantity} m²
                    </td>
                    <td className='px-4 py-5 font-semibold text-gray-900 dark:text-gray-100 whitespace-nowrap'>
                      €{fmt(e.result?.grandTotalWithTax)}
                    </td>
                    <td className='px-4 py-5 text-gray-500 dark:text-gray-400 text-sm whitespace-nowrap'>
